...
Students compare interpolation and polynomial fitting, then use residual plots,
model complexity, stability, and observations not used for fitting to identify
overfitting risk or a poor model choice.
...
- explain why training residuals alone cannot confirm that a model will work on
  new observations;
- compare candidate models on observations deliberately withheld from fitting;
...
| 50-65 min | Student task: compare degree 1, 2, and 7 fits |
...
| Time | Activity |
|---:|---|
| 0-8 min | Retrieval: residual evidence and the three question types |
| 8-20 min | Interpolation: input checks, `interp1`, and method choice |
| 20-32 min | Compare interpolation with polynomial fitting |
| 32-47 min | Student task: complete and explain the interpolation starter |
| 47-62 min | New-data check: fit on one subset and check on withheld observations |
| 62-70 min | AI critique and repair: high-degree polynomial overclaim |
| 70-75 min | Exit ticket: why is the smallest fit RMSE insufficient, and what check is stronger? |

### A First New-Data Check

Residuals calculated on the observations used to fit a model describe training
fit. They can reveal structure that the model missed, but they cannot by
themselves establish that a flexible model will work on new observations.

In Meeting 2, students return to the curve-fitting starter and divide the
...
Each candidate degree is fitted using only `x(fit_idx)` and `y(fit_idx)`. Its
RMSE is then calculated separately on the fit observations and on the withheld
check observations. The same split must be used for every candidate model.

One small split provides stronger evidence than training residuals alone, but
it is not a guarantee: the conclusion can depend on which observations were
held back. Students must not repeatedly redesign the model to make this one
check set look favorable.

For now, the course calls these **fit data** and **check data not used for
fitting**. Weeks 12-13 introduce the formal terms **training data** and
**validation data** and explain why the model plan must be frozen before the
validation outcomes are seen.
...
- Explain that residuals on the fitted observations are not a new-data check.
- Compare fit and check RMSE using a split declared before model fitting.
...
- When the question concerns new observations, reserve check observations
  before fitting and report their error separately from training error.
...
Project 1 is previewed this week because curve fitting, residuals, and RMSE are
its principal technical foundation. Students inspect the public question,
report template, data dictionary, and required filenames, and confirm their
group number. No group-specific data or project analysis is due at this stage.
...
[Week 3 exercises](week-03-exercises.qmd). The
